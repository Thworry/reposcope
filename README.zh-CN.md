...
完成的报告会先展示一个由八个区域组成的 README 优先证据档案，让读者在查看分数前先理解项目文档如何介绍自己，以及仓库公开证据能够确认什么：

1. **项目定位**：展示公开仓库说明和有界的项目简介；
2. **社区与维护事实**：Stars、Watchers、Forks、未关闭 Issue 与 PR、最后推送时间和许可证证据；
3. **读者结论**：在展开证据前概括文档所述的能力轮廓、典型路径、整体实现轮廓和采用边界；
4. **README 如何介绍项目**：按原始语言组织 README 的概览、目标读者、待解决问题、使用场景、依赖、限制和成熟度说明；
5. **核心能力**：按 README 原有主题展示最多九个能力分组；
6. **README 中的工作流程**：按文档顺序展示、即使没有连接线也能理解的步骤；
7. **README 声明与仓库观察**：并列展示 README 中的声明和仓库树中观察到的广义项目类型、技术生态与源代码区域；
8. **RepoScope 解读**：以“**值得注意**”“**依赖前需要核实**”和“**对实际使用意味着什么**”区分文档信号、未知项和使用影响。

GitHub 的 `stargazers_count`、`subscribers_count` 和 `forks_count` 分别提供 Stars、Watchers 和 Forks；`subscribers_count` 标记为 **Watchers**。GitHub 的 `open_issues_count` 同时包含 Issue 与 PR。这些数字反映公开关注程度。流行度不能证明项目质量或安全性。

...
4. 先查看项目速览，再查看总分、可信度、六个维度、优点、改进项、检查范围以及带文件链接的证据。
...
对于任何被检查的公开仓库，确定性的项目速览会区分用途证据与项目类型证据。用途证据来自公开 GitHub 仓库说明和首选 README。项目类型证据来自对清单、主题和仓库文件树的有界结构检查。证据链接固定到被检查的提交，仓库作者提供的用途文字保留源语言。项目速览不使用 AI 服务，也不是个性化建议：它不会推测私人需求，也不会声称某个仓库适合特定用户。
